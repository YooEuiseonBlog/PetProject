	...
	$.ajax({
		...
		success: function(result){
			if($("#user_name").val()==''){
				...
				$("#user_name").focus();
			}else if($("#user_name").val()=='${vo.user_name}'){
				error[3].innerHTML = "동일한 닉네임 입니다";
				error[3].style.display = "block";
				error[3].style.color = "red";
				$("#user_name").focus();
...
$(()=>{
	...
	var error = document.querySelectorAll('.error_alert');
	$("#memberEdit_frm").submit(function(){
		// 비밀번호 유효성검사
...
<div class="join_container">
	<form id="memberEdit_frm" method="post" action="/member/memberEditOk" enctype="multipart/form-data">
		<div class="join_page" id="page1">
		<br>
		<h2>회원 정보 수정</h2>
		<br>
			<div class="input_box"> 아이디<br>
			<input type="text" name="user_id" id="user_id" class="input02" onkeyup="idCheckInit()" value="${vo.user_id}" disabled/>
			<p class="error_alert"></p>
			</div>
			<div class="input_box">
			<input type="password" name="user_pwd" id="user_pwd" class="input02" value="${vo.user_pwd}" placeholder="비밀번호 입력"/>
			<p class="error_alert"></p>
			</div>
			
			<div class="input_box">
			<input type="password" id="user_pwdchk" class="input02" value="${vo.user_pwd}" placeholder="비밀번호 확인"/>
			<p class="error_alert"></p>
			</div>
			<div class="input_box">
			<input type="text" name="user_name" id="user_name" class="input02" onkeyup="nameCheckInit()" value="${vo.user_name}" placeholder="닉네임 입력"/>
			<input type="button" onclick="nameCheck()" class="button03" value="중복 검사">
			<input type="hidden" id="nameCheckResult" value="1"/>
			<p class="error_alert"></p>
			</div>
			
			<div class="input_box2">
				<input type="text" id="postcode" class="input02" placeholder="우편번호" disabled>
				<input type="button" onclick="execDaumPostcode()" class="button03" value="우편번호"><br>
				<input type="text" id="address" class="input03" value="${vo.user_addr}" name="user_addr" placeholder="주소">
...
<script>
    ...
    var element_img = document.getElementById('user_profileimg');
